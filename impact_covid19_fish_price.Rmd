...
ggplot(dat2 %>% filter(month %in% c(1:4)), aes(y = reorder(species,price_kg,min), x = log10(price_kg))) + 
  geom_boxplot() + 
...
```{r,fig.height=10,fig.width=5}
ggplot(dat2 %>% filter(month %in% c(1:4)), aes(y = reorder(species,landing_t,min), x = log10(landing_t))) + 
